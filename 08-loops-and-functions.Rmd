...
Great, so we can see that for values of `weight` where a number was recorded, we see an adjusted value but for values of `weight` where a number was not entered, it returns an `NA`.
